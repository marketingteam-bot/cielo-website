...
// .wrangler/tmp/pages-siWtuU/functionsWorker-0.39372818650443053.mjs
...
async function onRequestPost({ request, env: env2 }) {
  const json = /* @__PURE__ */ __name2((body, status = 200) => new Response(JSON.stringify(body), { status, headers: { "content-type": "application/json", "cache-control": "no-store" } }), "json");
  let data;
  try {
  ...
  if (data.website) return json({ ok: true, spam: true });
  const clean = /* @__PURE__ */ __name2((v, n = 500) => typeof v === "string" ? v.trim().slice(0, n) : "", "clean");
  const lead = {
    ts: (/* @__PURE__ */ new Date()).toISOString(),
    pilot: clean(data.pilot, 60),
    ...
    skus: clean(data.skus, 40),
    category: clean(data.category, 120),
    notes: clean(data.notes, 2e3),
  ...
  if (env2.LEAD_WEBHOOK) {
    try {
      const r = await fetch(env2.LEAD_WEBHOOK, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(lead), redirect: "follow" });
      if (!r.ok) {
        console.error("lead webhook failed", r.status, JSON.stringify(lead));
        return json({ ok: true, stored: false });
      }
    } catch (e) {
      console.error("lead webhook error", String(e), JSON.stringify(lead));
      return json({ ok: true, stored: false });
    }
...
// .wrangler/tmp/bundle-HmE7oS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HmE7oS/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.39372818650443053.js.map
